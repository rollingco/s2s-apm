...
$DEFAULTS = [
  'order_id' => isset($_GET['order_id']) ? (string)$_GET['order_id'] : ('orangemoney-' . time()),
  'amount'   => isset($_GET['amount'])   ? (string)$_GET['amount']   : '1.00',
  'currency' => isset($_GET['currency']) ? (string)$_GET['currency'] : 'SLE',
  'brand'    => isset($_GET['brand'])    ? (string)$_GET['brand']    : 'lgc-orange-money-dbm',
  'desc'     => isset($_GET['desc'])     ? (string)$_GET['desc']     : 'Orange Money payout test',
...
if ($submitted) {
  ...
  if ($brand === '') $brand = 'lgc-orange-money-dbm';
...
function render_page($ctx){
  ...
  $errors = $ctx['errors'] ?? [];
  $prefill= $ctx['prefill'] ?? ['order_id'=>'','amount'=>'','currency'=>'SLE','brand'=>'lgc-orange-money-dbm','desc'=>'','phone'=>'','email'=>''];
  $debug  = $ctx['debug'] ?? [];
  $resp   = $ctx['response'] ?? [];
...
<head>
<meta charset="utf-8">
<title>CREDIT2VIRTUAL — Orange Money payout</title>
<style>
...
<div class="wrap">
  ...
  <div class="panel">
    <div class="h">💸 Create CREDIT2VIRTUAL payout (Orange Money)</div>
    <form action="<?=h($self)?>" method="post">
      ...
      <div style="margin:8px 0;">
        ...
        <input type="text" name="order_id" value="<?=h($prefill['order_id'])?>" placeholder="e.g. orangemoney-123456">
      </div>
      ...
      <div style="margin:8px 0%;">
        <label>brand:</label>
        <input type="text" name="brand" value="<?=h($prefill['brand'])?>" placeholder="lgc-orange-money-dbm">
        <div class="small">Orange Money payout brand (default: lgc-orange-money-dbm).</div>
      </div>

      <div style="margin:8px 0%;">
        <label>description:</label>
        <input type="text" name="desc" value="<?=h($prefill['desc'])?>" placeholder="Orange Money payout test">
      </div>
